...
This analysis compares the current MC-5 API surface with the MC-6 cockpit vision. MC-6.1 through MC-6.11 and MC-6.13 Phases 2/3/4A/4B, fixture/live 4C presentation, 4C.1, 4C.2, 4C.3, 4D, and 4E are implemented and published. This document remains the compatibility and gap reference for the completed read-only surface and the explicitly blocked action plane. The current checkpoint is `1c1cc4d8839d122f46eb8a1c7592c9c504df68ba`.

The MC-6 API surface remains read-only and preserves existing route names and response semantics. The private authenticated `POST /api/advisor/evaluate` boundary is landed in Phase 4B, while the live read-only `GET /api/advisor` path is landed through server-owned orchestration, telemetry-owned export, the Phase 4D adapter, and deterministic composition. Phase 4E adds only bounded `resource_history_summary` presentation data. Other POST, PUT, PATCH, DELETE, acknowledgement, activation, retry, restart, update, shell, or remediation endpoints remain **FUTURE** because MC-6.12 operational execution is blocked. Cloudflare Access remains the selected perimeter boundary when enabled; AIPM does not verify Cloudflare JWTs or identity headers. The advisor remains non-authoritative and performs no actions, remediation, LLM/provider execution, or autonomous behavior.
