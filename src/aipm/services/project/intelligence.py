...
class ProjectIntelligenceService:
    ...
    def inventory(self, *, limit: int = MAX_PROJECTS, search: str | None = None, status: str | None = None) -> ProjectInventory:
        ...
        applications = sorted(applications, key=lambda item: item.display_name.lower())[:bounded]
    ...
    def _aggregate(
        ...
        for project in projects[: self.MAX_PROJECTS]:
            ...
            evidence = []
            if group:
                confidence = AssociationConfidence.EXACT if group == project.name else AssociationConfidence.PROBABLE
                evidence.append(ProjectEvidence("PROJECT_RUNTIME_MATCH", "info", "docker", f"Runtime group matched to {project.name}"))
            else:
                confidence = AssociationConfidence.UNKNOWN
                evidence.append(ProjectEvidence("PROJECT_RUNTIME_UNMATCHED", "warning", "association", "No trustworthy runtime group was matched"))
            if project_error:
                evidence.append(ProjectEvidence("PROJECT_DISCOVERY_UNAVAILABLE", "warning", "project", project_error))
            app = self._application(project.name, ProjectSource.DISCOVERED, confidence, project, group, selected, observed_at, evidence, runtime_error)
            result.append(app)
        ...
        for group in runtime_groups:
            ...
            result.append(self._application(group, ProjectSource.RUNTIME_GROUP, AssociationConfidence.UNKNOWN, None, group, selected, observed_at, evidence, runtime_error))
        ...
        if ungrouped:
            ...
            result.append(self._application("Ungrouped runtime", ProjectSource.UNGROUPED, AssociationConfidence.UNKNOWN, None, None, ungrouped, observed_at, evidence, runtime_error))
        return result
    ...
    def _matching_group(self, project: Project, components: list[Any]) -> str | None:
        names = {project.name, project.name.lower()}
        for item in components:
            group = getattr(item, "project_key", None)
            if group and str(group).lower() in names:
                return str(group)
        return None
