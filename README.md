...
The current Mission Control checkpoint is **MC-6.13 Phase 4B complete** at commit `af1a10b1f150335df27fda5d915f44e4f14146f4`. MC-6.13 Phase 2 established immutable evidence normalization, Phase 3 established the pure deterministic advisor rule engine, Phase 4A established pure bounded composition over those seams, and Phase 4B established the private authenticated read-only advisor evaluation API. The API accepts only bounded caller-supplied input, fails closed when authentication is unavailable or rejected, and does not collect live observations or perform actions. Phases 4C–4E have not started and remain unauthorized. The cockpit continues to cover overview, host/server intelligence, Docker, projects/applications, Systemd observations, bounded redacted Logs, history, events, incidents, and notification posture. MC-6.9 remains a separate design/inspection milestone, and production/runtime changes remain separate approval-gated operations.
...
MC-6.13 Phases 2, 3, 4A, 4B, fixture-only 4C presentation, and 4D are complete and pushed. Phase 2 normalizes bounded caller-supplied observations into immutable evidence bundles. Phase 3 applies ten deterministic, evidence-linked rules over immutable evidence using a canonical field schema and bounded `ResourceHistoryEnvelope` continuity contract. Phase 4A validates bounded immutable request metadata, recursively snapshots caller input, and directly composes the existing normalizer and rule engine. Phase 4B adds only the private authenticated `POST /api/advisor/evaluate` transport boundary: it validates bounded JSON, reconstructs typed history envelopes, delegates directly to `AdvisorCompositionRequest` and `compose_advisor()`, and returns the existing `AdvisorResponse` with safe 400/401/422/500 transport behavior. Phase 4C adds only the fixture-driven, non-live `#/ai-agent` presentation, and Phase 4D adds only the telemetry-owned bounded export and transport-neutral adapter. Cloudflare Access is the selected edge authentication boundary for the documented public ingress; AIPM relies on private edge protection and does not implement JWT verification, identity middleware, session storage, or proxy-header trust. The advisor remains non-authoritative and performs no live collection, LLM/provider work, or autonomous action. Live Phase 4C orchestration, stronger application identity behavior, and Phase 4E remain separately authorized and not started. See [`docs/MC-6.13_STATUS.md`](docs/MC-6.13_STATUS.md).
